...
const EditEvent = () => {
    ...
    const fetchEvent = async () => {
        try {
            ...
            setFormData({
                ...
                max_participants: ev.max_participants, status: ev.status
            });
            if (ev.banner_image) setPreview(ev.banner_image);
        } catch (error) {
    ...
    if (initLoading) return <div className="text-center p-20"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white mx-auto"></div></div>;

    return (
        <div className="page-container max-w-3xl">
            <h1 className="text-2xl font-medium text-white mb-8 pb-4 border-b border-[#2e2e2e]">Edit Event</h1>

            <form onSubmit={handleSubmit} className="space-y-6">

                {/* Same layout as CreateEvent, keeping it DRY for this step */}
                <div>
                    ...
                    <div className="relative h-64 border border-dashed border-[#2e2e2e] rounded-md overflow-hidden hover:border-[#666] transition-colors group bg-[#0a0a0a] flex items-center justify-center">
                        {preview ? (
                            ...
                            <div className="text-center">
                                <p className="text-sm font-medium text-[#a1a1a1] group-hover:text-white transition-colors">Click to update image</p>
                ...
                <div className="pt-8 flex justify-end border-t border-[#2e2e2e] mt-4">
                    ...
                    <button type="submit" className="btn-primary py-2 px-8 text-sm flex justify-center w-36" disabled={loading}>
                        {loading ? <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-black"></span> : 'Save Changes'}
                    </button>
                </div>
            </form>
        </div>
